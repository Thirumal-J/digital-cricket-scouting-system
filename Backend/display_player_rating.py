...
def trainbatsman():
    ...
    for count in range(3):
        ...
        if(count==0):
            train_batsman["Performance"]=(0.026*train_batsman["Inn"])+(0.063*train_batsman["Runs"])+(0.23*train_batsman["Ave"])+(0.232*train_batsman["SR"])+(0.15*train_batsman["4s"])+(0.154*train_batsman["6s"])+(0.024*train_batsman["NO"])+(0.027*train_batsman["50"])+(0.023*train_batsman["100"])+(0.047*train_batsman["Balls"])-(0.023*train_batsman["0"])
        elif(count==1):
            train_batsman["Performance"]=(0.021*train_batsman["Inn"])+(0.068*train_batsman["Runs"])+(0.23*train_batsman["Ave"])+(0.232*train_batsman["SR"])+(0.15*train_batsman["4s"])+(0.154*train_batsman["6s"])+(0.024*train_batsman["NO"])+(0.027*train_batsman["50"])+(0.023*train_batsman["100"])+(0.047*train_batsman["Balls"])-(0.023*train_batsman["0"])
        else:
            train_batsman["Performance"]=(0.03*train_batsman["Inn"])+(0.059*train_batsman["Runs"])+(0.23*train_batsman["Ave"])+(0.232*train_batsman["SR"])+(0.15*train_batsman["4s"])+(0.154*train_batsman["6s"])+(0.024*train_batsman["NO"])+(0.027*train_batsman["50"])+(0.023*train_batsman["100"])+(0.047*train_batsman["Balls"])-(0.023*train_batsman["0"])
        train_batsman["Performance"]=10*(train_batsman["Performance"]/max(train_batsman["Performance"]))
        ...
        train_batsman['Performance'] = train_batsman['Performance'].astype('int')
...
def fetchRatingsbatsman(manualflag):
    ...
    if(manualflag==0):
        #X_test=pd.read_csv('Dataset/Test_Batsman.csv')
        ...
        for row in datatable:
            ...
            y_pred_toporder=model_toporder.predict([[Inn,NO,Runs,Ave,Balls,SR,hundreds,fifties,ducks,fours,sixes]])
            y_pred_midorder=model_midorder.predict([[Inn,NO,Runs,Ave,Balls,SR,hundreds,fifties,ducks,fours,sixes]])
            y_pred_finisher=model_finisher.predict([[Inn,NO,Runs,Ave,Balls,SR,hundreds,fifties,ducks,fours,sixes]])
            ...
            sql="update tbl_batsmanauction set toporder_rating="+ str(y_pred_toporder) +", midorder_rating="+str(y_pred_midorder)+",finisher_rating="+(y_pred_finisher)+" where player_name='"+playerName+"'"
            cur.execute(sql)
        ...
        new_prediction_midorder=ratingsConvert_batsman(y_pred_midorder)
        new_prediction_finisher=ratingsConvert_batsman(y_pred_finisher)
        y_pred_toporder=" ".join(str(x) for x in y_pred_toporder)
        y_pred_midorder=" ".join(str(x) for x in y_pred_midorder)
        y_pred_finisher=" ".join(str(x) for x in y_pred_finisher)
        response={
...
def trainbowler():
    ...
    train_bowler["Performance"]=(0.041*train_bowler["Inn"])+(0.056*train_bowler["Overs"])+(0.364*train_bowler["Wkts"])-(0.318*train_bowler["Econ"])+(0.056*train_bowler["Mdns"])+(0.099*train_bowler["4w"])+(0.066*train_bowler["5w"])
    train_bowler["Performance"]=train_bowler["Performance"]+ abs(min(train_bowler.Performance))
    train_bowler["Performance"]=10*(train_bowler["Performance"]/max(train_bowler["Performance"]))


    for index,row in train_bowler.iterrows():
        train_bowler["Performance"][index]=math.ceil(train_bowler["Performance"][index])
    ...
    train=train_bowler.drop('Performance', axis=1).drop(['Ave','SR','Runs','Mat'], axis=1)
...
def fetchRatingsbowler(manualflag):
    ...
    if(manualflag==0):
        ...
        for row in datatable:
            ...
            y_pred_bowler=model.predict([[Inn,Overs,Wkts,Econ,Mdns,fourwickethaul,fivewickethaul]])
            

            y_pred_bowler=" ".join(str(x) for x in y_pred_bowler)

            sql="update tbl_bowlingauction set rating="+ str(y_pred_bowler) +" where player_name='"+playerName+"'"
            cur.execute(sql)
...
def basepricegraph(baseprice):
    ...
    conn =psycopg2.connect(dbname=DB_NAME,user=DB_USER,password=DB_PASS,host=DB_HOST)
    cur=conn.cursor()
    query="select baseprice,count(*) from tbl_batsmanauction where baseprice<"+baseprice+"group by baseprice order by baseprice"
    cur.execute(query)
